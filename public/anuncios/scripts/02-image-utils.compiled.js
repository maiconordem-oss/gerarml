...
async function removeBgChroma(dataUrl, opts = {}) {
  ...
  for (let i = 0; i < d.length; i += 4) {
    ...
    if (dist < tolerance) {
      ...
      d[i + 3] = Math.round(d[i + 3] * (dist - tolerance) / (edgeFeather * 10));
    }
  }
...
async function removeBgVision(dataUrl) {
  ...
  if (!visionResp.ok) {
    ...
    throw new Error(err.error?.message || 'Erro Vision API');
  }
...
async function improveQuality(dataUrl) {
  ...
  if (!resp.ok) {
    ...
    throw new Error(err.error?.message || 'Erro Images API');
  }
